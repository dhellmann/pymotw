...
``sys.excepthook`` to a function that takes three arguments (error
...
exception is raised the following ``print`` statement is not run, even
...
The return value of ``exc_info()`` is a three member tuple containing
...
.. note:: 
    ...
    traceback and a local variable in the current frame by ignoring
    that part of the return value from ``exc_info()``.  If you do need
    the traceback for some reason (such as to log it), you should
    explicitly delete the local variable (using ``del``) when you're
    done to avoid cycles.
...
variables in :mod:`sys` (``last_type``, ``last_value``, and
``last_traceback``) to make it easy to retrieve them for debugging.
With the post-mortem debugger in :mod:`pdb` you don't have to use the
values directly.
