...
The :mod:`smtpd` module includes classes for building simple mail transport protocol servers.  It is the server-side of the protocol used by :mod:`smtplib`.
...
The simplistic example above shows the arguments to ``process_message()``, but :mod:`smtpd` also includes a server specifically designed for debugging, called DebuggingServer.  It prints the entire incoming message to stdout and then stops processing it (it does not proxy the message to a real mail server).
